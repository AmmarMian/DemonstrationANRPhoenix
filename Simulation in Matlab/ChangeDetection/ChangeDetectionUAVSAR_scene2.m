...
%% Reading Image
    ...
    ScenePosition = [3236,25601,3836,27901];
...
%     Results_Pol = computeCDOnImages(ImageHyperCube, PolDetectorsStruct);
    ...
    % Compute results
    hbar = parfor_progressbar(numberOfWorkers,'Computing Pol ...');  %create the progress bar
...
%% ROC
    load('groundTruth_UAVSARScene2')
    groundTruth_UAVSARScene2 = groundTruth_UAVSARScene2(floor(My/2)+1:end-floor(My/2), floor(Mx/2)+1:end-floor(Mx/2));
    numberOfPoints = 100;
    figPol = figure;
    markers = {'s', 'd', 'o', '*', '+', '^'};
    lg = {};
    ...
    % Polarimetry
    lg = {'$\mathrm{P}_{\mathrm{D}}=\mathrm{P}_{\mathrm{FA}}$'};
    ...
    for d=1:PolDetectorsStruct.number
        ...
        lgComparison = cat(2, lgComparison, PolDetectorsStruct.list(d).name);
        ...
        for i_lambda=1:numberOfPoints 
            Pd(i_lambda) = sum(sum(groundTruth_UAVSARScene2.*(Res > lambda(i_lambda)))) / sum(groundTruth_UAVSARScene2(:));
            Pfa(i_lambda) = sum(sum(not(groundTruth_UAVSARScene2).*(Res > lambda(i_lambda)))) / sum(sum(not(groundTruth_UAVSARScene2)));
        end
        figure(figPol)
    ...
    xlim([1e-5,1])
    [Mx, My] = size(PolDetectorsStruct.mask);
    textString = {sprintf('$N_1=%d$, $N_2=%d$', Mx, My)};
    annotation('textbox',...
    [0.15, 0.57, 0.1, 0.1], ...
    'Units','characters',...
    'String', textString,...
...
%         colorbar
    ...
    for d=1:PolDetectorsStruct.number
        [Mx, My] = size(PolDetectorsStruct.mask);
        toShow = zeros(Ny,Nx);
        toShow(floor(My/2)+1:end-floor(My/2), floor(Mx/2)+1:end-floor(Mx/2)) = log(abs(Results_Pol{d}));
        toShow(toShow==0) = nan;
